...
recordStream._transform = function(line, encoding, done) {
    var logRecord = line.toString();
    var serializedRecord = logRecord;
...
function s3LogsToES(bucket, key, context) {
    ...
    s3Stream
      ...
      .on('data', function(parsedEntry) {
          //console.log('JSON BODY: ' + parsedEntry);
          //console.log('TIMESTAMP: ' + JSON.stringify(JSON.parse(parsedEntry).timestamp));
...
function postDocumentToES(doc, context) {
    ...
    req.method = 'POST';
    req.path = path.join('/', esDomain.index, esDomain.doctype, timestamp.toString());
    console.log('POSTING TO: ' + req.path);
